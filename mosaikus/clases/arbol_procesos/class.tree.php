...
class tree
{
	...
	public function mk($parent, $position = 0, $data = array(),$id_organizacion = null) {
		$parent = (int)$parent;
	...
	public function mv($id, $parent, $position = 0,$id_organizacion) {
		$id			= (int)$id;
		$parent		= (int)$parent;
		if($parent == 0 || $id == 0 || $id == 1) {
			throw new Exception('Cannot move inside 0, or move root node');
		}
                /*valida que no se pueda mover al nivel 2 si tiene mas de un area seleccionada*/
                if ($parent == 2 && !(strpos($id_organizacion, ',')===false)){
                        throw new Exception('Cannot move inside 0, or move root node id_organizacion');
                }
		...
		";
                ...
                // asigna id_organizacion
                if ((strpos($id_organizacion, ',')===false)){
                    $sql[] = 
                             "
                            UPDATE ".$this->options['structure_table']."
                                    SET ".$this->options['structure']["id_organizacion"]." = $id_organizacion
                            WHERE				
                                    ".$this->options['structure']["id"]." IN(".$id[$this->options['structure']["id"]].")
                    ";
                }
	...
	public function rm($id) {
		$id = (int)$id;
                $numero_registros = $this->db->one("SELECT COUNT(*) FROM mos_registro_item WHERE tipo = 12 AND valor = $id");
                if ($numero_registros+0>0){
                    return array('exito' => 2, 'msj' => 'No se puede eliminar el proceso, existen Registros asociados');
                }                
		...
		foreach($sql as $v) {
			try {
				...
				throw new Exception('Could not remove');
                                ...
                                return array('exito' => 2, 'msj' => 'Error al eliminar');
			}
		}
