...
if(isset($_GET['operation'])) {
	$fs = new tree(db::get($cadena_conn), array('structure_table' => 'mos_arbol_procesos', 'data_table' => 'mos_arbol_procesos_nombres', 'data' => array('title')));
	try {
		$rslt = null;
...
////				//}                                
			...
			case 'create_node':
				$node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int)$_GET['id'] : 2;
                                if (strpos($_GET['id_organizacion'], ',')===false){                                    
                                    $temp = $fs->mk($node, isset($_GET['position']) ? (int)$_GET['position'] : 0, array('title' => isset($_GET['text']) ? $_GET['text'] : 'New node'),$_GET['id_organizacion']);
                                }
                                else{                                    
			...
			case 'move_node':
				$node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int)$_GET['id'] : 0;
				$parn = isset($_GET['parent']) && $_GET['parent'] !== '#' ? (int)$_GET['parent'] : 2;
				$rslt = $fs->mv($node, $parn, isset($_GET['position']) ? (int)$_GET['position'] : 0, $_GET['id_organizacion']);
				break;
			case 'copy_node':
				$node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int)$_GET['id'] : 0;
