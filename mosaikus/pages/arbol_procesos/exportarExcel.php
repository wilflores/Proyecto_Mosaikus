...
//        }         
        ...
        for($i=1;$i<=$params[niveles];$i++){
            ...
            switch ($i) {                                             
                case 1: 
                    $objActSheet->setCellValueByColumnAndRow($i-1,6,$pagina->nombres_columnas['arbol_organizacional']);
                    break;
                case 2: 
                    $objActSheet->setCellValueByColumnAndRow($i-1,6,$pagina->nombres_columnas['proceso']);
                    break;
                case 3: 
                    $objActSheet->setCellValueByColumnAndRow($i-1,6,$pagina->nombres_columnas['actividad']);
                    break;
                default:
                    $ind++;
                    $objActSheet->setCellValueByColumnAndRow($i-1,6,$pagina->nombres_columnas['sub_actividad']." ". ($ind));
                    break;
           }
            
            //$objActSheet->setCellValueByColumnAndRow($i-1,6, $pagina->nombres_columnas['nivel']." ". ($i+2)); 
            ...
            foreach ($data as $value) { 
               // print_r($value);
                ...
                {   if (($i==1)||($value["nombre_$i"]<>'')){
                    ...
                    if($i==1)
                        $objActSheet->setCellValue($columnas[$i-1].$row, str_replace('&#8594;','->',$ao->BuscaOrganizacional(array('id_organizacion' => $value["id_".($i)]))));
                    else
