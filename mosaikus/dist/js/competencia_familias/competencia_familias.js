    ...
    function nuevo_Familias(){
            array = new XArray();
            array.setObjeto('Familias','crear');
            ...
            array.addParametro('import','clases.competencia_familias.Familias');
    ...
    function validar(doc){        
        if($('#idFormulario').isValid()) {
            ...
            if(i==0){// para validar que al menos deben agregar una categoria
                VerMensaje('error','Debe Agregar al menos familia');
                return;
            }
            ...
            if (doc.getElementById("opc").value == "new")
                array.setObjeto('Familias','guardar');
            else
                array.setObjeto('Familias','actualizar');
            array.addParametro('modo',document.getElementById('modo').value);            
            array.addParametro('cod_link',document.getElementById('cod_link').value);
    ...
    function editarFamilias(id){
        array = new XArray();
        array.setObjeto('Familias','editar');
        array.addParametro('modo',document.getElementById('modo').value);            
        array.addParametro('cod_link',document.getElementById('cod_link').value); 
    ...
    function eliminarFamilias(id){
        if(confirm("¿Desea Eliminar la Familia Seleccionada?")){
            array = new XArray();
            array.setObjeto('Familias','eliminar');
            array.addParametro('id',id);
            array.addParametro('permiso',document.getElementById('permiso_modulo').value);
            array.addParametro('import','clases.competencia_familias.Familias');
            xajax_Loading(array.getArray());
    ...
    function verPagina(pag,doc){
        ...
        array.setObjeto('Familias','buscar');
        array.addParametro('import','clases.competencia_familias.Familias');
        $('#MustraCargando').show();
        xajax_Loading(array.getArray());
    }

    function verFamilias(id){
        var src = 'pages/' +  document.getElementById("modulo_actual").value + '/verFamilias.php?id='+id;
    ...
    /**************** funciones para familias-items adaptacion de plantilla inspecciones**/
    /*agregue esta funcion - RAQUEL 13-07-16 */
        ...
        html = html + '<td class="td-table-data">'+
                            '<input id="vigencia_din_'+ i + '" checked class="form-control" type="checkbox" name="vigencia_din_'+ i + '">'+
                       '</td>';
        html = html + '<td class="td-table-data">'+
                            '<input id="codigo_din_'+ i + '" class="form-control" type="text" data-validation="required" name="codigo_din_'+ i + '">'+
                       '</td>';
        html = html + '<td>'+
                            '<input id="descripcion_din_'+ i + '" class="form-control" type="text" data-validation="required" name="descripcion_din_'+ i + '">'+
                       '</td>';   
