...
class _tree_struct {
	...
	// Constructor
	function __construct($table = "mos_organizacion", $fields = array()) {
		$this->table = $table;
		if(!count($fields)) {
	...
	function _remove($id) {
	 	//return false;
	 	//ANTES DE ELIMINAR DEBE VALIDAR QUE NO EXISTA NINGUN REGISTRO ASOCIADO AL NODO O ALGUNO DE SUS HIJOS.....

		if ($this->table=="mos_organizacion") {
			$this->db->query("Select (Select Count(*) From mos_documentos_estrorg_arbolproc Where id_organizacion_proceso in (Select id From mos_organizacion Where (id=$id Or parent_id=$id)) )+
							(Select Count(*) From mos_accidentes_ley Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_cierre_mes_niveles Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_incidentes Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_matrices Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_matrices_relaciones_cabecera Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_personal Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_personal_capacitacion Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_recordatorio_ingreso Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) +
							(Select Count(*) From mos_registro Where id_organizacion in (Select id From mos_organizacion Where (id=$id Or parent_id=$id))) as nro_registros");
			$valor = 0;
			while($this->db->nextr()) {
				$valor = $this->db->f(0);
			}
...
class json_tree extends _tree_struct {
	function __construct($table = "mos_organizacion", $fields = array(), $add_fields = array("title" => "title", "type" => "type")) {
